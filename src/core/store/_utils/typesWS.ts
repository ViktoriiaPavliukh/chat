// Actions
export interface IActionType<Type extends string = string> {
  TYPE: Type;
}
...
export interface IAction<Type extends string = string> {
  type: Type;
}

export interface IPayloadAction<Payload, Type extends string = string>
  extends IAction<Type> {
  payload: Payload;
}

export interface IErrorAction<Error, Type extends string = string>
  extends IAction<Type> {
  error: Error;
}

// Action creators
export type IActionCreator<Type extends string = string> =
  (() => IAction<Type>) & IActionType<Type>;

export type IPayloadActionCreator<Payload, Type extends string = string> = ((
  payload: Payload
) => IPayloadAction<Payload, Type>) &
  IActionType<Type>;

export type IErrorActionCreator<Error, Type extends string = string> = ((
  error: Error
) => IErrorAction<Error, Type>) &
  IActionType<Type>;
...
export type IAnyActionCreator<Payload> =
  | IActionCreator
